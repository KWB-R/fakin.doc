...
1. Names should not cause problems during automated data processing. 

2. Names should be meaningful to you and your colleagues. You and they should be
able to guess what a folder or file contains so that required files are found 
easily. 

3. Names should not cause problems when copying files within your (computer) 
system or between different systems, especially between different operating 
systems or systems in different countries.

We process files automatically. Listing or reading files with non-standard 
characters in their names very often causes problems. These problems are 
sometimes not easy to  solve. But they are easy to avoid: By sticking to some 
simple restrictive rules when naming files and folders.

In order to hit the first target we define the following set of allowed 
characters:
...
See the FAQ chapter to answer the following questions:
...
Instead of space, use underscore, hyphen or "camel case":

* Use underscore to separate different parts of the file or folder name, as in
  `results_today`, `results_tomorrow`, `input_site-1`, `output_site-1`.

* Use hyphen or "camel case" (as in `camelCase`) instead of space or underscore
  to visually separate the parts of compound words or names, e.g.
  
    + `site-1` instead of `site 1` or `site_1`, 
    + `dissolved-oxygen` or `dissolvedOxygen` instead of `dissolved oxygen` or 
    `dissolved_oxygen`,
    + `clean-data` or `cleanData` instead of `clean data` or `clean_data`.
...
underscores and hyphens to separate words e.g.
...
We distinguish between rules specifying

* a set of allowed characters,
* a set of allowed words (vocabulary) that may appear as part of a file or 
folder,
* the order in which we expect the words to appear in a file or folder name.

It is useful if your department or project team agrees on the 
following elements of a file name: 

* Vocabulary: Choose a standard vocabulary for file names, so that everyone 
uses a common language

We aggreed on coding **dates** in file or folder names in one of these forms:

* `yyyy-mm-dd` (e.g. `2018-06-28`)
* `yyyymmdd` (e.g. `20180628`)

When dates are coded like so, files names only differing in the date by default
display chronologically in the file explorer or in file listings. This is 
because by default files are sorted alphanumerically by their name. 
