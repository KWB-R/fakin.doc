...
find files easily. It is useful if your department or project agrees on the 
...
characters very often causes problems. These problems are sometimes not easy to 
solve. But they are easy to avoid: By sticking to some simple restrictive rules
when naming files and folders.

Restricting to characters from the base character set defined above set avoids 
problems when exchanging files between partners in different countries. In the 
same way we have problems with their files, e.g.

```
Example file from Canasoft жк. Хаджи Димитър, ул.Клисура_Troncons.xml
``` 

they will have problems with our files such as
...
Using hyphen to separate the year, month and day parts of the date supports
to distinguish these parts visually. Using the hyphen instead of the underscore
allows to split a file name at underscores without splitting the date into 
parts. 

Using hyphen or "camel case" for composed words will not split the composed words
into its parts when splitting the file or folder name at underscores.
